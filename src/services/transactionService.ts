...
export const transferFunds = async (
    ...
    try {
        ...
        // Deduct from sender immediately to block liquidity
        sender.accountBalance -= amount;
        await sender.save({ session });

        // NOTE: Credit to receiver removed from here. 
        // Settlement happens only after Admin Approval.
        ...
        const debitTransaction = await Transaction.create([{
            ...
            type: 'debit',
            description: description || (protocol === 'Domestic'
                ? `Transfer to ${receiverName}`
                : `${protocol} Transfer to ${receiverName}`),
            detailLabel: `${protocol} // OUTBOUND`
        }], { session });

        // Credit record is NOT created until approval to keep recipient ledger clean
        // It will be generated by the adminController upon status change to 'success'

        // Send informative alert to admin for all transfers
        sendAdminAlert(
            `[Optima Flow] ${sender.name} - ${commonData.currency} ${amount.toLocaleString()}`,
            `<p style="margin-top: 0;">A new fund transfer requires institutional review.</p>
             <div style="margin: 20px 0; padding: 20px; border: 1px solid #eeeeee; background: #fafafa;">
                ...
                <p style="margin: 5px 0 0 0; font-size: 13px;"><strong>Recipient:</strong> ${receiverName} (${receiverAccountNumber})</p>
